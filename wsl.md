...
Windows の方では普通にアクセスできるウェブアクセスがそれなりの頻度で失敗する or
やたら時間がかかる。
調べると DNS の応答がなんだか怪しい。

どうやら DNS 周りは昔からトラブルが多いらしく、
今は WSL Tunneling 機能がデフォルトで有効になっているらしい。
これは仮想ネットワークの中に怪しいアドレスの DNS サーバがおり、
問い合わせると Windows での問い合わせにつないでくれる (トンネリング) ようだ。

しかしこれでもまだ怪しいことがあるようだ…。
Windows のファイアウォールに引っかかっているという噂も。

少々乱暴かもしれない直し方

トンネル仮想 DNS サーバが書かれた `resolve.conf` の自動生成をやめさせる。

```sh
# WSL 全体に設定したい場合は Windows の `%USERPROFILE%\.wslconfig`
